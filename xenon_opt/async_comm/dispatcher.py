...
from xenon_opt.structure import Job
...
class Dispatcher(object):
    ...
    def __init__(self, new_result_callback, run_id='0',
        ...
        new_result_callback: function
            function that will be called with a `Job instance <xenon_opt.async_comm.dispatcher.Job>`_ as argument.
        ...
        self.pyro_id = "xenon_opt.run_%s.dispatcher" % self.run_id
    ...
    def discover_workers(self):
        ...
        while True:
            ...
            with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
                worker_names = ns.list(prefix="xenon_opt.run_%s.worker." % self.run_id)
