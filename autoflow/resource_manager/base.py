...
class ResourceManager(StrSignatureMixin):
    ...
    def insert_experiment_record(
        ...
        self.experiment_id = self._insert_experiment_record(
            self.user_id, getattr(self, "hdl_id", None), self.task_id,
    ...
    def get_trial_model(self) -> pw.Model:
        class Trial(pw.Model):
            ...
            user_id = pw.IntegerField()
            config_id = pw.FixedCharField(max_length=32,null=True)
            config = self.JSONField(default={},null=True)  # new
            config_info = self.JSONField(default={},null=True)  # new
            # run_id = pw.FixedCharField(max_length=256)
            # instance_id = pw.FixedCharField(max_length=128)
            experiment_id = pw.IntegerField()
            task_id = pw.FixedCharField(max_length=32, index=True,null=True)  # 加索引
            hdl_id = pw.FixedCharField(max_length=32,null=True)
            estimator = pw.CharField(max_length=256, default="",null=True)
            loss = pw.FloatField(default=ERR_LOSS,null=True)
            losses = self.JSONField(default=[],null=True)
            test_loss = self.JSONField(default=[],null=True)
            all_score = self.JSONField(default={},null=True)
            all_scores = self.JSONField(default=[],null=True)
            test_all_score = self.JSONField(default={},null=True)
            models_path = pw.TextField(default="",null=True)
            final_model_path = pw.TextField(default="",null=True)
            y_info_path = pw.TextField(default="",null=True)
            additional_info = self.JSONField(default={},null=True)
            # smac_hyper_param = PickleField(default=0)
            dict_hyper_param = self.JSONField(default={},null=True)
            cost_time = pw.FloatField(default=ERR_LOSS,null=True)
            status = pw.CharField(max_length=32, default="SUCCESS",null=True)
            failed_info = pw.TextField(default="",null=True)
            warning_info = pw.TextField(default="",null=True)
            intermediate_results = self.JSONField(default=[],null=True)
    ...
    def insert_trial_record(self, info: Dict):
        ...
        return self._insert_trial_record(self.user_id, self.task_id, self.hdl_id, self.experiment_id,
                                         info)
    ...
    def _insert_trial_record(self, user_id: int, task_id: str, hdl_id: str, experiment_id: int,
                             info: dict):
        success = False
        max_try_times = 3
        trial_id = -1
