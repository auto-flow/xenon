...
class Worker(object):
    ...
    def __init__(
        ...
        self.worker_id = "neutron.run_%s.worker.%s.%i" % (self.run_id, socket.gethostname(), os.getpid())
    ...
    def _run(self):
        ...
        try:
            with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
                ...
                dispatchers = ns.list(prefix="neutron.run_%s.dispatcher" % self.run_id)
    ...
    def compute(self, config_id, config, config_info, budget, working_directory):
        ...
        if self.eval_func is None:
            raise NotImplementedError(
                "Subclass neutron.distributed.worker and overwrite the compute method in your worker script")
        kwargs = {}
